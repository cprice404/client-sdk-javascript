...
interface RequestMultipleKeysLoggingFormat {
  keys: string[];
}

// Currently used for Get, Delete, ItemGetTtl, and ItemGetType requests
interface RequestSingleKeyLoggingFormat {
  key: string;
}
...
interface SetRequestLoggingFormat {
  key: string;
  value: string;
  ttl_milliseconds: number;
}

export function convertSetRequest(
...
export function convertGetBatchRequest(
  request: cache.cache_client._GetBatchRequest
): GetBatchRequestLoggingFormat {
  return {
