...
export function runGetSetDeleteTests(
  ...
  describe('when configured to throw on errors', () => {
    it('throws a FailedPreconditionError if increment is called on a string', async () => {
      ...
      await expect(async () => {
        await delay(10);
        // throw new FailedPreconditionError('FUCK YOU');
        console.log('about to call the bad increment');
        const incrementResponsePromise = cacheClientWithThrowOnErrors.increment(
          integrationTestCacheName,
          field,
          1
        );
        console.log('about to await the bad increment');
